                            ...
                            <div class="row">
                                ...
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <select v-model="form.cat_id" class="form-control" id="exampleFormControlSelect1"
                                            v-on:change="onChangesub(this.form.cat_id)">
                                ...
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <select v-model="form.sub_cat_id" class="form-control"
                                            id="exampleFormControlSelect1">
                                            <option v-for="(sub_cats, index) in sub_cat" :value="sub_cats.id">{{
                                                sub_cats.name }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-3">
                                ...
                                <div class="col-3">
                                    <div class="form-group">
                                        ...
                                        <input type="number" v-model="form.dis_price" class="form-control"
                                            placeholder="Enter Category Name">
                                    </div>
                                </div>
                                ...
                                <div class="col-2">
                                    <div class="form-group">
                                        ...
                                        <button style="margin-top: 32px; margin-left: 28px;" type="button"
                                            class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-xl"
                                            @click="getMedia(1)">Upload Image</button>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="form-group">
                                                ...
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        <DropZone :maxFiles="Number(10000000000)"
                                                            url="http://127.0.0.1:8000/api/filestore" :uploadOnDrop="true"
                                                ...
                                                <div class="col-12">
                                                    <div class="form-group">
                                                        <ul class="images_wrapper" v-if="medias">
                                                            <li class="img_box" v-for="(media, index) in medias.data"
                                                                :key="index">
                                                                <img class="collegecardimage rounded "
                                                                    v-bind:src="'/media/' + media.name" />
                                                                <div class="image_name">{{ media.original_name }}</div>
                                                                <div class="images_ex">{{ media.extension }}</div>
                                                                <div class="overlay">
                                                                    <a href="#" @click.prevent="deleteMedia(media.id)"
                                                                        class="icon" title="Delete Image">
                                                                        <i class="fa fa-trash"></i>
                                                                    </a>
                                                                    <a href="#" data-dismiss="modal"
                                                                        @click.prevent="selectMedia(media.name)"
                                                                        class="icon2" title="Select Image">
                                                                        <i class="fa fa-trash"></i>
                                                                    </a>
                                                        ...
                                                        <Bootstrap4Pagination :data="medias"
                                                            @pagination-change-page="getMedia" />
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="col-2" v-if="selected != ''" style="margin-top:22px;">
                                    <div class="form-group">
                                        <button type="button" @click="multipledelete" class="btn btn-danger">Delete
                                            Multiple</button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table">
                                ...
                                <tbody v-if="s_products == '' || s_products == []">
                                    <tr v-for="(product, index) in products.data" :key="index">
                                        ...
                                        <td><img :src="'/media/' + product.feturer_image" alt="image" width="100"
                                                height="100" /></td>
                                        <td><label class="switch">
                                                <input type="checkbox" v-if="product.status == 0"
                                                    @change="active_deactive([product.id, 1])">
                                                <input type="checkbox" v-else="product.status == 1"
                                                    @change="active_deactive([product.id, 0])" checked>
                                                <span class="slider round"></span>
                                            </label>
                                        </td>
                                        <td>
                                            <a @click.prevent="edit(product.id)" type="button" class="btn btn-primary"
                                                href="#" data-toggle="modal" data-target="#exampleModal"><i
                                                    class="fa fa-edit" title="Edit"></i></a>
                                            <a @click.prevent="deleteproduct(product.id)" class="btn btn-danger" href="#"><i
        ...
        edit(id) {
            ...
            axios.get('http://127.0.0.1:8000/api/editproduct/' + id)
                .then((response) => {
                    ...
                    CKEDITOR.instances.editor1.setData(response.data.product_des, function () { this.updateElement(); })
                })
        }
    }
}
